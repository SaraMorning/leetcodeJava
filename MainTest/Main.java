...
public class Main {
    static Calendar c = new GregorianCalendar(2020, GregorianCalendar.MAY, 10);
    ...
    int[] arrInt1 = new int[] {1,2,5};
    Integer[] arrInteger1 = Arrays.stream(arrInt1).boxed().toArray(Integer[]::new);
    List<Integer> list1 = Arrays.stream(arrInt1).boxed().collect(Collectors.toList());

    int[] arrInt2 = Arrays.stream(arrInteger1).mapToInt(Integer::valueOf).toArray();
    List<Integer> list2 = Arrays.asList(arrInteger1);

    int[] arrInt3 = list1.stream().mapToInt(Integer::intValue).toArray();
    Integer[] arrInteger2 = list1.stream().toArray(Integer[]::new);

    List<Integer> list4 = Arrays.stream(arrInt1).boxed().collect(Collectors.toList());
    list4.sort(Comparator.comparingInt(Integer::intValue).reversed());
    int i = 0;
    }

    private static void test11(String[] args) {

        System.out.printf(args[0] + "did not match ... on %terd", c);
    }
